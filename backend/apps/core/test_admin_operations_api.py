...
class AdminOperationsAPITests(TestCase):
    def setUp(self) -> None:
        ...
        self.allowed_client = Client.objects.create(
            name="Allowed Contact", company="Allowed Client", email="allowed@example.test"
        )
        self.hidden_client = Client.objects.create(
            name="Hidden Contact", company="Hidden Client", email="hidden@example.test"
        )
        ClientAccessGrant.objects.create(
            profile=self.profile, client=self.allowed_client, granted_by=self.staff
        )
        ...
        Task.objects.create(ownership_type=OwnershipType.INTERNAL, title="Internal Task")
    ...
    def test_client_list_respects_object_scope(self) -> None:
        ...
        self.assertEqual([client["company"] for client in response.json()], ["Allowed Client"])
    ...
    def test_lead_list_requires_permission_then_returns_pipeline(self) -> None:
        ...
        self.assertEqual([lead["company"] for lead in allowed.json()], ["Lead Company"])
    ...
    def test_server_register_requires_permission_and_returns_inventory(self) -> None:
        denied = self.client.get("/api/admin/infrastructure/servers")
        self.assertEqual(denied.status_code, 403)

        self.grant("infrastructure", "view_server")
        allowed = self.client.get("/api/admin/infrastructure/servers")

        self.assertEqual(allowed.status_code, 200)
        self.assertEqual(len(allowed.json()), 1)
        self.assertEqual(allowed.json()[0]["hostname"], "operations-test.example.test")
