                ...
                <div className="space-y-6">
                        ...
                        {resource.legacy_reference?.fields.length ? (
                            <dl className="mt-5 grid gap-x-6 gap-y-5 sm:grid-cols-2">
                                {resource.legacy_reference.fields.map((field) => (
                                    <div key={field.key} className={field.kind === "multiline" ? "sm:col-span-2" : ""}>
                    ...
                    <Card className="p-5">
                        ...
                        {resource.relationships.length === 0 ? (
                            <p className="mt-5 text-sm text-slate-500">No visible resource relationships yet.</p>
                        ) : (
                            <div className="mt-4 divide-y divide-slate-800">
                                {resource.relationships.map((relationship) => (
                                    <Link
                                        ...
                                        className="flex flex-col gap-1 py-3 transition hover:bg-slate-900/50 sm:flex-row sm:items-center sm:justify-between sm:px-2"
                                    >
                                        <div>
                ...
                <div className="space-y-6">
                    <Card className="p-5">
                        ...
                        <dl className="mt-4 space-y-4 text-sm">
                            <div>
                                ...
                                <dd className="mt-1 text-slate-300">{resource.client_name || "ADB Internal"}</dd>
                            </div>
                            <div>
                                <dt className="text-xs text-slate-600">Resource type</dt>
                                <dd className="mt-1 text-slate-300">{label(resource.resource_type)}</dd>
                            </div>
                            <div>
                    ...
                    {resource.legacy_reference ? (
                        <Card className="border-amber-950/70 bg-amber-950/10 p-5">
                            ...
                            <p className="mt-2 text-xs leading-5 text-amber-200/60">
                                This resource is currently backed by the existing {label(resource.legacy_reference.legacy_type)} record #{resource.legacy_reference.legacy_id}. The structured identity is now the ownership and relationship anchor; the old row remains intact during migration.
                            </p>
                        </Card>
                    ) : null}
                    ...
                    <Card className="p-5">
                        ...
                        <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                            {["Credentials", "Monitoring", "Documentation", "Activity"].map((item) => (
                                <div key={item} className="rounded-lg border border-slate-800 bg-slate-950/50 px-3 py-3 text-slate-500">
                                    {item}
                                </div>
                            ))}
                        </div>
