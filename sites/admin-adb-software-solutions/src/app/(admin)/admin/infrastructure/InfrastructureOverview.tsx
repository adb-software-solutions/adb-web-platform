...
const inventoryLinks = [
    ...
    [
        ...
        "Managed and self-hosted database instances.",
    ],
    [
        "Websites",
        "/admin/infrastructure/websites",
        "Production, staging and development web properties.",
    ],
    [
        "Domains",
        "/admin/infrastructure/domains",
        "Registrations, renewals and DNS-linked properties.",
    ],
    [
        "SSL certificates",
        "/admin/infrastructure/ssl-certificates",
        "Certificate expiry and provider inventory.",
    ],
    [
        "Licences",
        "/admin/infrastructure/licences",
        "Subscriptions, renewals and software licences.",
    ],
    [
        "Applications",
        "/admin/infrastructure/applications",
        "Logical applications spanning infrastructure components.",
    ],
    [
        "Technology stack",
...
const additionalSystems = [
    [
        "Mobile apps",
        "/admin/infrastructure/mobile-apps",
        "mobile_app_count",
        DevicePhoneMobileIcon,
    ],
    ...
    [
        "Email systems",
        "/admin/infrastructure/email-systems",
        "email_system_count",
        EnvelopeIcon,
    ],
] as const;
    ...
    return (
        <div className="space-y-6">
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <StatCard
                    label="Servers"
                    value={String(summary.server_count)}
                    helper="Tracked compute hosts"
                    icon={<ServerStackIcon className="h-5 w-5" />}
                />
                <StatCard
                    label="Databases"
                    value={String(summary.database_count)}
                    helper="Database instances"
                    icon={<CircleStackIcon className="h-5 w-5" />}
                />
                <StatCard
                    label="Websites"
                    value={String(summary.website_count)}
                    helper="Web properties"
                    icon={<GlobeAltIcon className="h-5 w-5" />}
                />
                <StatCard
                    label="Applications"
                    value={String(summary.application_count)}
                    helper="Logical applications"
                    icon={<CloudIcon className="h-5 w-5" />}
                />
                <StatCard
                    label="Domains"
                    value={String(summary.domain_count)}
                    helper={`${summary.expiring_domain_count} expire within 45 days`}
                    icon={<GlobeAltIcon className="h-5 w-5" />}
                    accent={
                        summary.expiring_domain_count > 0 ? "amber" : "cyan"
                    }
                />
                <StatCard
                    label="SSL certificates"
                    value={String(summary.ssl_certificate_count)}
                    helper={`${summary.expiring_certificate_count} expire within 45 days`}
                    icon={<KeyIcon className="h-5 w-5" />}
                    accent={
                        summary.expiring_certificate_count > 0
                            ? "amber"
                            : "green"
                    }
                />
                <StatCard
                    label="Licences"
                    value={String(summary.licence_count)}
                    helper={`${summary.renewing_licence_count} renew within 45 days`}
                    icon={<KeyIcon className="h-5 w-5" />}
                    accent={
                        summary.renewing_licence_count > 0 ? "amber" : "cyan"
                    }
                />
                <StatCard
                    label="APIs & bots"
                    value={String(summary.api_count + summary.bot_count)}
            ...
            <div className="grid gap-4 lg:grid-cols-[2fr_1fr]">
                <Card className="p-5">
                    <div className="mb-4">
                        ...
                        <p className="mt-1 text-xs text-slate-500">
                            Jump into a register to inspect the underlying
                            operational records.
                        </p>
                    </div>
                ...
                <Card className="p-5">
                    ...
                    <div className="mt-4 space-y-2">
                        {additionalSystems.map(
                            ([label, href, countKey, Icon]) => (
                                <Link
                                    key={href}
                                    href={href}
                                    className="flex items-center justify-between rounded-lg px-2 py-2 text-sm transition hover:bg-slate-900"
                                >
